...
Begin
  ...
  If Copy(Str,1,5) = 'MSGID' Then Begin
    ...
    MsgHdr^.JamHdr.MsgIdCrc := StringCRC32(TmpStr);
  End Else
...
Begin
  ...
  JamIdx^[IdxLoc - JM^.IdxStart].MsgToCrc := StringCRC32(JM^.MsgTo);
...
Begin
  ...
  If WriteError = 0 Then Begin             {Handle message text}
    ...
    If WriteError = 0 Then             {Add index record}
      ...
      TmpIdx.MsgToCrc := StringCRC32(JM^.MsgTo);
...
Begin
  JM^.YourName   := Name;
  JM^.YourHdl    := Handle;
  JM^.NameCrc    := StringCRC32(Name);
  JM^.HdlCrc     := StringCRC32(Handle);
  JM^.CurrMsgNum := JM^.BaseHdr.BaseMsgNum - 1;
