...
Begin
  ...
  // A39: stamp a FidoNet MSGID kludge when posting via NNTP to an echomail
  // (non-QWK) base, matching how bbs_msgbase/mutil_common generate MSGIDs.
  If (TempBase.NetType > 0) and (TempBase.QwkNetID = 0) Then
    MsgBase^.DoKludgeLn (#1 + 'MSGID: ' + Addr2Str(MsgBase^.GetOrigAddr) + ' ' + strI2H(CurDateDos, 8));
