...
Procedure ProtocolStatusDraw (Upload: Boolean);
Var
  Box : TMenuBox;
Begin
  Box := TMenuBox.Create(TOutput(Screen));
  ...
  If Upload Then
    Box.Header := ' Zmodem Upload '
  Else
    Box.Header := ' Zmodem Download ';
...
Procedure ProtocolStatusUpdate (P: AbstractProtocolPtr; First, Last: Boolean);
Var
  KBRate : LongInt;
Begin
...
Begin
  ...
  ProtocolStatusDraw(True);

  Protocol := New(ZmodemProtocolPTR, Init(Client, False));

  Protocol^.MakeFileList  (FileList, 1024 * 8);
  Protocol^.AddFileToList (FileList, FileName);
  Protocol^.SetDestinationDirectory(XferPath);
  Protocol^.SetFileList(FileList);
  Protocol^.StatusInterval := 25;
  Protocol^.SetShowStatusProc(@ProtocolStatusUpdate);
