...
Var
  TempPath          : String;
  smtpRefuseForeign : Boolean = True;   // runtime SMTP setting (data/mis.ini)
//  Term     : TTermAnsi;
...
// Load runtime MIS settings.  RefuseForeign now lives in the config editor
// (Config -> SMTP Server -> Allow Foreign); a legacy data/mis.ini, if present,
// overrides it for backward compatibility.
Procedure LoadMISConfig;
Var
  Ini : TIniReader;
  FN  : String;
Begin
  // Source of truth is the config editor: Config -> SMTP Server -> Allow Foreign.
  // inetSMTPAllowForeign FALSE (default) = refuse foreign for unknown users (A38
  // behavior); TRUE = accept.  smtpRefuseForeign is the runtime inverse used by the
  // SMTP client.
  smtpRefuseForeign := Not bbsCfg.inetSMTPAllowForeign;

  // Backward compatibility: if a legacy data/mis.ini exists, let it OVERRIDE the
  // config setting (older installs configured RefuseForeign there).  We no longer
  // auto-create this file - the setting now lives in the config editor.
  FN := bbsCfg.DataPath + 'mis.ini';
  If FileExist(FN) Then Begin
    Ini := TIniReader.Create(FN);
    smtpRefuseForeign := Ini.ReadBoolean('SMTP', 'RefuseForeign', smtpRefuseForeign);
    Ini.Free;
  End;
End;
