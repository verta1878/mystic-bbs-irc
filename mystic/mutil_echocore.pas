...
//                   IsNetMail := True;
  ...
  For Count1 := 1 to MsgLines Do Begin
    If (MsgText[Count1]^ = '') or (MsgText[Count1]^[1] = #1) or
       (Copy(MsgText[Count1]^, 1, 7) = 'SEEN-BY') Then Continue;

    For Count2 := 1 to Length(MsgText[Count1]^) Do
      MsgCRC.Text := Crc32(Byte(MsgText[Count1]^[Count2]), MsgCRC.Text);
  End;
End;
