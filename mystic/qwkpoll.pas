...
// Write a status line to the console AND append it (timestamped) to
// qwkpoll.log in the logs directory, so a poll can be diagnosed after the fact
// (previously qwkpoll only wrote to the console, which is lost after the run).
// Mirrors the fidopoll.log pattern.
...
Begin
  ...
  Assign (TF, bbsCfg.LogsPath + 'qwkpoll.log');
  ...
  If IoResult = 0 Then Begin
    WriteLn (TF, FormatDate(CurDateDT, 'NNN DD HH:II') + ' ' + Str);
    Close   (TF);
  End;
End;
...
Begin
  ...
  PrintLog('- Exchanging Mail for ' + QwkNet.Description);
  ...
  PrintLog('   - Exported @' + QwkNet.PacketID + '.rep -> ' + strI2S(QWK.TotalMessages) + ' msgs');
  PrintLog('   - Connecting via FTP to ' + QWkNet.HostName);
  ...
  If FTP.OpenConnection(QwkNet.HostName) Then Begin
    ...
    If FTP.Authenticate(QwkNet.Login, QwkNet.Password) Then Begin
      PrintLog('   - Logged in as ' + QwkNet.Login);

      If QWK.TotalMessages > 0 Then Begin
        PrintLog('   - Sending reply packet');

        Case FTP.SendFile(QwkNet.UsePassive, TempPath + QwkNet.PacketID + '.rep', QwkNet.PacketID + '.rep') of
          ftpResOK      : SentFile := True;
          ftpResBadData : PrintLog('      - Unable to open data connection');
        Else
          PrintLog('      - Failed');
        End;
      End;

      PrintLog('   - Downloading QWK packet');

      Case FTP.GetFile(QwkNet.UsePassive, TempPath + QwkNet.PacketID + '.qwk') of
        ftpResOK      : PrintLog('      - OK: ' + QwkNet.PacketID + '.qwk (' + strComma(FileByteSize(TempPath + QwkNet.PacketID + '.qwk')) + ' bytes)');
        ftpResFailed  : PrintLog('      - Failed');
        ftpResBadData : PrintLog('      - Unable to open data connection');
        ftpResNoFile  : PrintLog('      - Packet does not exist on remote');
      End;
    End;
  End;
  ...
  If (QWK.TotalMessages > 0) and Not SentFile Then Begin
    PrintLog('   - Send of REP failed; reseting export pointers');
  ...
  If FileExist(TempPath + QwkNet.PacketID + '.qwk') Then Begin
    ...
    PrintLog('   - Importing QWK packet');

    If QWK.ImportPacket(True) Then
      PrintLog('   - Imported ' + strI2S(QWK.RepOK) + ' messages (' + strI2S(QWK.RepFailed) + ' failed)')
    Else
      PrintLog('   - Unable to find QWK packet');
  End;
...
Var
  ...
  User : RecUser;
Begin
  PrintLog('- Importing ' + Path + QwkNet.PacketID + '.qwk');
...
Var
  ...
  User : RecUser;
Begin
  PrintLog('- Exporting ' + Path + QwkNet.PacketID + '.rep');
  ...
  PrintLog('   - Exported ' + strI2S(QWK.TotalMessages) + ' messages');
...
Begin
  ...
  PrintLog('Program session start at ' + FormatDate(CurDateDT, 'NNN DD YYYY HH:II:SS'));
  WriteLn;
