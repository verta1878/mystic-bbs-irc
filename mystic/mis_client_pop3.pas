...
Begin
  ...
  While MsgBase^.SeekFound Do Begin
    ...
    if IsUser(MsgBase^.GetTo) Then Begin
      ...
      AddLine ('Date: ' + FormatDate(DateDos2DT(DateStr2Dos(MsgBase^.GetDate + ' ' + MsgBase^.GetTime)), 'DDD, DD NNN YYYY HH:II:SS'));
...
Begin
  If LoggedIn Then Begin
    ...
    Client.WriteLine(re_OK + strI2S(DataMsg) + ' ' + strI2S(Datasize));
  End Else
  ...
  If LoggedIn Then Begin
    ...
    If Data <> '' Then Begin
      ...
      If (MsgNum > 0) and (MsgNum <= MailSize) and (Not MailInfo[MsgNum]^.Deleted) Then
        Client.WriteLine(re_OK + strI2S(MsgNum) + ' ' + strI2S(MailInfo[MsgNum]^.MsgSize))
      Else
        Client.WriteLine(re_UnknownMail);
    End Else Begin
      GetMessageCount(MsgNum, MsgSize);

      Client.WriteLine (re_OK + strI2S(MsgNum) + ' messages (' + strI2S(MsgSize) + ' octets)');

      For Count := 1 to MailSize Do
        If Not MailInfo[Count]^.Deleted Then
          Client.WriteLine (strI2S(Count) + ' ' + strI2S(MailInfo[Count]^.MsgSize));
