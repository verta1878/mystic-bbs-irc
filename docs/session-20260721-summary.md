...
### Printer API Research
- FPC RTL `printer` unit — text-only Lst variable, all platforms
- Lazarus OSPrinters — CUPS/Win32 GDI/Carbon/Qt
- Missing: ESC/P, PCL, PostScript from console apps
- Common API designed: prnapi.pas + driver units (8.3 names)
- 300 DPI starting point, scales to 600/1200
- Pipeline: RIP framebuffer → scale to DPI → driver → printer

### HTML 1.0 for RIPx
- RFC 1866 HTML 2.0 as baseline (~40 tags)
- HTML → RIP command mapping documented
- Parser architecture: htmlpars → htmltree → htmllayo → htmlrip
- Entities mapped to CP437 codepoints
- Integrates with print API (same pixel buffer)

### BBS Software Status
- FrontDoor: FREE + maintained (user knows author)
- Doorway: possibly dead (user knows author)
- ACiDDraw: skip — use CIADraw (in examples/, Pascal source)
- RemoteAccess → EleBBS (FPC clone) proves the TP→FPC port model
- PCBoard 13.x released, v15 PPL scripting is all ASM (bytecode VM approach needed)
- Mystic BBS: active, g00r00's legacy, our upstream

### Architecture Discussion
- Pure Pascal approach: no ASM needed for DOS hardware
- FPC provides Port[], Intr(), MsDos() — covers LPT, SB, VGA, keyboard
- HAL not needed — Port[] IS the abstraction
- Goal: recreate dead DOS software in maintainable Pascal
- 8 targets from one codebase

### DOS 8.3 Filename Compliance
- 15 codec files exceed 8-char limit
- Audit and rename list documented in ripv4printwiteup.md
...
|---------|------|----------|
| fpc264irc-r31-repo-phase10-20260721.zip | 375MB | Full repo, v3 complete |
| fpc-codecs-standalone-gplv3-20260721.zip | 480KB | 60 codec units standalone |
| fpc264irc-v4-preview-20260721.zip | 15KB | Unicode/TTF + printer writeup + HTML spec |

### v4 Preview (held separate)
- cp437utf8.pas (201) — CP437↔UTF-8 translation table
- utf8render.pas (279) — Bitmap glyph text renderer
- ttfglyph.pas (386) — TTF/OTF parser + rasterizer
- RIPSCRIPT_V4_ROADMAP.md — Whitepaper gap analysis
- ripv4printwiteup.md (506) — Printer API + HTML 1.0 spec

---

## Final Stats
- 121 source files in codec package
- 60 codec units (+ pasjpeg 58 files)
- 26,402 lines of codec code
- All compile on x86_64-linux + i386-linux + i386-win32 + i386-go32v2
- Zero deferred codec phases remain
- v4 content separated from v3 source

---

## TODO (not codec-related)
- Phase 11: VM testing (DOSBox VESA+SB, QEMU, VirtualBox)
- DOS LCL RIP Browser (Phase A-F in roadmap)
- Post codecs to PabloDraw / ripapi maintainer
- Try compiling xqtr's Blocker/BlockArt/xAnsiViewer
- Update CODEC_REFERENCE.md with all new units
- 8.3 filename rename pass
- Connect with FrontDoor / Doorway authors about fpc264irc

---

*7 years and counting. For evga/g00r00. For the scene. o7*
